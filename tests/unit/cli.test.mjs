...
test("installed init owns the complete persisted discovery and semantic continuation", async () => {
  ...
  try {
    ...
    const fakeSource = [
      ...
      "      : (() => { const manifest = JSON.parse(readFileSync('semantic-manifest.json','utf8')); return {schema:'assistant.bootstrap-output/v1',project_summary:{purpose:null,scope:null,current_state:'Observed existing project',current_authorization:null,authorization_state:'not_authorized',authorized_work:[],blocked_work:['Unspecified work'],authorization_basis_paths:[],next_safe_route:'Ask the user for direction'},candidate_nodes:[],coverage_groups:[{selector_kind:'exact_path',selector:'README.md',disposition:'preserved',reason:'Root orientation document accounted for'}],semantic_coverage:manifest.units.map((unit) => ({unit_id:unit.unit_id,disposition:'observed_noncanonical',target_ids:[],reason:'Synthetic nonsemantic installed-init fixture'})),document_assets:manifest.document_assets.map((asset) => ({path:asset.path,observed_role:'artifact',proposed_action:'not_document_asset',proposed_destination:null,target_ids:[],reason:'Synthetic nonsemantic installed-init fixture',requires_confirmation:false,decision_status:'not_required'})),legacy_surfaces:[],lineage:{origin_ids:[],ordered_stage_ids:[],current_ids:[],complete:true,missing:[]},closed_book_audit:{origin_to_current_explainable:true,current_authorization_explainable:true,hypotheses_explainable:true,decisions_explainable:true,live_legacy_dependencies:[],missing_concerns:[]},gaps:[],conflicts:[]}; })();",
